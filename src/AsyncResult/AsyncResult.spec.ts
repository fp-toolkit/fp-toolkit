...
describe("AsyncResult", () => {
    describe("constructors", () => {
        describe("Ok", () => {
            it("produces a new Ok", async () => {
                expect(
                    await pipe(AsyncResult.ok(30), Async.start)
                ).toStrictEqual(Result.ok(30))
            })
        })
        ...
        describe("Err", () => {
            it("produces a new Err", async () => {
                expect(
                    await pipe(AsyncResult.err("err"), Async.start)
    ...
    describe("bind", () => {
        ...
        it("returns a flattened Err if the bind projection returns an Err", async () => {
            ...
            const f1 = () => AsyncResult.ok<string, Error>("ok")
            const f2 = (_: string) =>
                AsyncResult.err<Error, number>(new Error("err"))
            // act
            const actual = await pipe(f1(), AsyncResult.bind(f2), Async.start)
            // assert
            expect(actual).toStrictEqual(Result.err(new Error("err")))
    ...
    describe("bindResult", () => {
        ...
        it("returns a flattened Err if the bind projection returns an Err", async () => {
            ...
            const f1 = () => AsyncResult.ok<string, Error>("ok")
            const f2 = (_: string) =>
                Result.err<Error, number>(new Error("err"))
            // act
            const actual = await pipe(
                f1(),
                AsyncResult.bindResult(f2),
    ...
    describe("tryCatch", () => {
        ...
        it("uses the onThrow function if given", async () => {
            ...
            // act
            const actual = await pipe(
                AsyncResult.tryCatch(f, onThrow),
                Async.start
            )
            // assert
    ...
    describe("start", () => {
        it("invokes the AsyncResult", async () => {
            expect(
                await pipe(AsyncResult.ok("A"), AsyncResult.start)
            ).toStrictEqual(Result.ok("A"))
        })
    })
