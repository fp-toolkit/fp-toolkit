...
export function tryCatch<A, E = unknown>(
    mightThrow: () => A,
    onThrow?: (ex: unknown) => E
): Result<A, any> {
    const toError = (ex: unknown) =>
        ex instanceof Error ? ex : Error(String(ex))

    try {
        return ok(mightThrow())
    } catch (ex) {
        if (onThrow != null) {
            return err(onThrow(ex))
        }
        return err(toError(ex))
    }
}
...
export const getEqualityComparer = <A, E>(
    ...
    EqualityComparer.ofEquals((r1, r2) => {
        if (
            isErr(r1) &&
            isErr(r2) &&
            equalityComparerE.equals(r1.err, r2.err)
        ) {
            return true
        }
