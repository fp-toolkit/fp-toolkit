...
describe("Array", () => {
    describe("choose", () => {
        it("projects and keeps only the Some results", () => {
            ...
            const actual = pipe(
                arr,
                Array.choose(
                    flow(Option.ofNullish, Option.map(globalThis.String))
                )
            )
            // assert
    ...
    describe("concatFirst", () => {
        it("adds the new values at the beginning of the partially applied values", () => {
            expect(
                pipe(["a", "b"], Array.concatFirst(["c", "d"]))
    ...
    describe("chunk", () => {
        ...
        it("returns even chunks if the array is evenly divisible by the chunk size", () => {
            expect(
                pipe([1, 2, 3, 4, 5, 6, 7, 8, 9], Array.chunk(3))
            ).toStrictEqual([
                [1, 2, 3],
                [4, 5, 6],
                [7, 8, 9],
        ...
        it.each([[-50], [0], [0.1442], [1.77]])(
            ...
            size => {
                expect(pipe([1, 2, 3], Array.chunk(size))).toStrictEqual([
                    [1],
                    [2],
                    [3],
                ])
    ...
    describe("flatten", () => {
        it("flattens a nested array", () => {
            expect(
                pipe([[1, 2], [3, 4], [5], [6], []], Array.flatten)
    ...
    describe("except", () => {
        ...
        it("returns the given array if the excludeThese array is empty", () => {
            expect(pipe([1, 2, 3], Array.except<number>([]))).toStrictEqual([
                1, 2, 3,
            ])
        })
    ...
    describe("getEqualityComparer", () => {
        it("always returns true if the arrays are both empty", () => {
            const { equals } = Array.getEqualityComparer(
